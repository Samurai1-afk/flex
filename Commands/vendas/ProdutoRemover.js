...
const run = async (client, message) => {
    ...
    if (produtos.length < 1) return message.reply({ embeds: [new MessageEmbed().setFooter("∙ Créditos | Humongous ©")
    .setAuthor({
      name: `Bot de Vendas`,
    ...
    menuCollector.on('collect', async i => {
        ...
        if (msgProduto) {
            ...
            try {
                ...
                await i.followUp({ embeds: [new MessageEmbed().setFooter("∙ Créditos | Humongous ©")
                .setAuthor({
                  name: `Bot de Vendas`,
                  iconURL: config.author}).setColor(config.color).setDescription(`Ocorreu um erro ao **remover** o produto da database.`)], ephemeral: true});
        ...
        .setAuthor({
          name: `Bot de Vendas`,
          iconURL: config.author}).setColor(config.color).setDescription(`Produto removido da database com **sucesso**.`)], ephemeral: true});
        msgMenu.delete();
